...
# Get sample data
result <- read.table(paste(pfx, '/genotype.txt', sep = ''), header = T, as.is = T, sep = '\t', comment.char = '', colClasses = c('character'))
...
plot_base <- function(r, ytitle) {
  plot(x = 0, type = 'n', xlim = c(tgt.reg.srt, tgt.reg.end), ylim = c(y_min * r, y_max * r), xlab = paste('Chromosome ', tgt.chr, sep =''),	ylab = '', yaxt = 'n')
  title(ylab = ytitle)
  ...
  if (asb == 'hg19') {
    ...
    add_gene_name(tgt.dat$hg38_start, tgt.dat$hg38_end, tgt.nam, r)
  }
  ...
  if (tgt.dat$paralog != '.') {
    ...
    if (asb == 'hg19') {
    	...
    	add_gene_name(prg.dat$hg38_start, prg.dat$hg38_end, prg.dat$name, r)
    }
...
add_gene_model <- function(g, r) {
	...
	if (asb == 'hg19') {
		...
		rect(xleft = as.numeric(unlist(strsplit(g$hg19_exon_starts, split = ','))), ybottom = y_min * 0.5 * r + 0.15 * r, xright = as.numeric(unlist(strsplit(g$hg19_exon_ends, split = ','))), ytop = y_min * 0.5 * r - 0.15 * r, col = 'black', border = 'black')
	} else {
		lines(x = c(g$hg38_start, g$hg38_end), y = c(y_min * 0.5 * r, y_min * 0.5 * r))
		rect(xleft = as.numeric(unlist(strsplit(g$hg38_exon_starts, split = ','))), ybottom = y_min * 0.5 * r + 0.15 * r, xright = as.numeric(unlist(strsplit(g$hg38_exon_ends, split = ','))), ytop = y_min * 0.5 * r - 0.15 * r, col = 'black', border = 'black')
	}
...
for (i in 1:nrow(result)) {
	...
	pdf(file = paste(pfx, '/plot/', id, '.pdf', sep = ''))
	par(mfrow = c(2, 2))
	...
	if (ctl.nam == '.') {
		add_gene_name(ctl.srt, ctl.end, "CUSTOM", sc1)
	} else {
	...
	af <- read.table(paste(pfx, '/af/', id, '.txt', sep = ''), header = T)
