...
ref <- arg[[8]] # reference sample list
asb <- arg[[9]] # reference assembly
...
if (asb == 'hg19') {
  tgt.gen.srt <- tgt.dat$hg19_start
  tgt.gen.end <- tgt.dat$hg19_end
  msk.srt <- tgt.dat$hg19_masked_starts
  msk.end <- tgt.dat$hg19_masked_ends
  
} else {
  tgt.gen.srt <- tgt.dat$hg38_start
  tgt.gen.end <- tgt.dat$hg38_end
  msk.srt <- tgt.dat$hg38_masked_starts
  msk.end <- tgt.dat$hg38_masked_ends
}

tgt.reg.srt <- tgt.gen.srt - tgt.dat$upstream
tgt.reg.end <- tgt.gen.end + tgt.dat$downstream

tgt.siz <- tgt.reg.end - tgt.reg.srt
...
tgt <- gdf[chr == tgt.chr & pos >= tgt.reg.srt & pos <= tgt.reg.end, ]
...
if (tgt.dat$hg19_masked_starts == '.') {
  msk.dat <- '.'
  msk.pos <- '.'
  ums <- cpn
} else {
  msk.dat <- as.data.frame(cbind(srt = as.integer(unlist(strsplit(msk.srt, split = ','))), end = as.integer(unlist(strsplit(msk.end, split = ',')))))
  msk.pos <- c()
...
prf <- as.data.frame(tgt.reg.srt:tgt.reg.end)
...
for (i in 1:length(svd$name)) {
  if (cp.all[i] == '.') {
    ...
    cp <- diff(c(tgt.reg.srt - 1, as.numeric(unlist(strsplit(cp.all[i], split = ','))), tgt.reg.end))
  }
...
for (i in 2:ncol(obs)) {
  ...
  write.table(results[order(ssr), ], file = paste(pfx, '/ssr/', colnames(obs)[i], '.txt', sep = ''), sep = '\t', quote = F, row.names = F)
}
...
save.image(paste(pfx2, '/sv.RData', sep = ''))
